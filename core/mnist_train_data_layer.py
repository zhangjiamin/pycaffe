...
class MNISTTrainDataLayer(Layer):
    ...
    def Forward_cpu(self, bottom, top):
        top[0].set_data( self.train_set_x[self.cur_:self.cur_+self.batch_size_]  )
        top[1].set_data( self.train_set_y[self.cur_:self.cur_+self.batch_size_]  )
        top0_shape = (self.batch_size_, 784)
        top1_shape = (self.batch_size_, 10)
        top[0].Reshape(top0_shape)
        top[1].Reshape(top1_shape)
        ...
        if self.cur_ >= self.train_set_x.shape[0]:
            self.cur_ = 0
